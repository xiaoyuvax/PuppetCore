...
public sealed class TaskBoardForm : Form, IPuppet
{
    ...
    private sealed record BoardTask(string Title, Priority Priority = Priority.Normal, bool Completed = false, DateTime CreatedAt = default)
    {
        ...
        public override string ToString() => $"{(Completed ? "已完成" : "待办")}   ·   {PriorityLabel()}   ·   {Title}";
        internal string PriorityLabel() => Priority switch { Priority.High => "高", _ => "普通" };
    }

    private readonly List<BoardTask> _tasks = [];
    private readonly TextBox _taskInput = new() { Name = "TaskInput", AccessibleName = "新任务标题", PlaceholderText = "下一件值得完成的小事…", Dock = DockStyle.Fill, MaxLength = 120 };
    private readonly ListBox _taskList = new() { Name = "TaskList", AccessibleName = "任务列表", Dock = DockStyle.Fill, IntegralHeight = false, BorderStyle = BorderStyle.None, ItemHeight = 34, HorizontalScrollbar = true, DrawMode = DrawMode.OwnerDrawFixed };
    private readonly ComboBox _filter = new() { Name = "TaskFilter", AccessibleName = "任务筛选", DropDownStyle = ComboBoxStyle.DropDownList, Dock = DockStyle.Fill };
    private readonly ComboBox _priority = new() { Name = "TaskPriority", AccessibleName = "任务优先级", DropDownStyle = ComboBoxStyle.DropDownList, Dock = DockStyle.Fill };
    private readonly Button _addButton = MakeButton("AddButton", "添加 (&A)", true);
    private readonly Button _completeButton = MakeButton("CompleteButton", "完成所选 (&C)");
    private readonly Button _removeButton = MakeButton("RemoveButton", "删除所选 (&D)");
    private readonly Button _renameButton = MakeButton("RenameButton", "重命名 (&R)");
    private readonly Button _priorityButton = MakeButton("PriorityButton", "切换优先级 (&P)");
    private readonly Button _clearDoneButton = MakeButton("ClearDoneButton", "清除已完成 (&E)");
    private readonly Label _counts = MakeLabel("TaskCounts", "", 11);
    private readonly Label _status = MakeLabel("BoardStatus", "从一件小事开始。任务仅保存在本次会话。", 10);
    private readonly Label _agentStatus = MakeLabel("AgentStatus", "Puppet 尚未启动", 9);
    ...
    public string CurrentFilter => _filter.SelectedItem?.ToString() ?? "全部";
    ...
    public TaskBoardForm()
    {
        ...
        var layout = new TableLayoutPanel { Name = "BoardLayout", AccessibleName = "任务看板布局", Dock = DockStyle.Fill, Padding = new Padding(28), ColumnCount = 1, RowCount = 8 };
        ...
        layout.Controls.Add(MakeLabel("Heading", "小步  /  TASK BOARD", 23, FontStyle.Bold), 0, 0);
        layout.Controls.Add(MakeLabel("Subtitle", "专注眼前，把计划变成已完成。", 10), 0, 1);

        var entry = new TableLayoutPanel { Name = "EntryLayout", AccessibleName = "添加任务", Dock = DockStyle.Fill, ColumnCount = 3, RowCount = 1, Margin = Padding.Empty };
        ...
        _priority.Items.AddRange(["普通", "高"]);
        _priority.SelectedIndex = 0;
        entry.Controls.Add(_priority, 1, 0);
        entry.Controls.Add(_addButton, 2, 0);
        layout.Controls.Add(entry, 0, 2);

        var toolbar = new TableLayoutPanel { Name = "FilterLayout", AccessibleName = "筛选与数量", Dock = DockStyle.Fill, ColumnCount = 2, RowCount = 1, Margin = Padding.Empty };
        toolbar.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100));
        toolbar.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 160));
        toolbar.Controls.Add(_counts, 0, 0);
        _filter.Items.AddRange(["全部", "待办", "已完成"]);
        _filter.SelectedIndex = 0;
        toolbar.Controls.Add(_filter, 1, 0);
        layout.Controls.Add(toolbar, 0, 3);
        layout.Controls.Add(_taskList, 0, 4);

        var actions = new FlowLayoutPanel { Name = "ActionLayout", AccessibleName = "所选任务操作", Dock = DockStyle.Fill, WrapContents = false, Padding = new Padding(0, 8, 0, 0), Margin = Padding.Empty };
        ...
        _addButton.Click += (_, _) => AddTask(_taskInput.Text, _priority.SelectedItem!.ToString());
    ...
    public bool ToggleSelectedPriority()
    {
        if (_taskList.SelectedItem is not BoardTask task)
            return Report("请先选择任务。", false);
        if (task.Completed) return Report("已完成的任务不再调整优先级。", false);
        ...
        RenderTasks(toggled);
        return Report(next == Priority.High ? "已标记为高优先级。" : "已恢复为普通优先级。", true);
    }
    ...
    public int ClearCompletedTasks()
    {
        ...
        if (removed == 0)
        {
            Report("当前没有已完成的任务。", false);
            return 0;
        }
        _tasks.RemoveAll(task => task.Completed);
        RenderTasks();
        Report($"已清除 {removed} 项已完成任务。", true);
        return removed;
    }
    ...
    public bool RemoveSelectedTask()
    {
        ...
        _tasks.Remove(task);
        RenderTasks();
        return Report("已删除任务。", true);
    }

    public bool SetFilter(string? filter)
    {
        if (filter is not ("全部" or "待办" or "已完成"))
            return Report("筛选必须为：全部、待办或已完成。", false);
        if (CurrentFilter != filter)
        {
            _filter.SelectedItem = filter;
            return true;
        }
        RenderTasks(_taskList.SelectedItem as BoardTask);
        return Report(VisibleCount == 0 ? "当前筛选下没有任务。" : $"正在显示{filter}任务。", true);
    }
    ...
    private void RenderTasks(BoardTask? selected = null)
    {
        ...
        _counts.Text = $"{TotalCount} 项任务   ·   {PendingCount} 待办（{HighPriorityPendingCount} 高优）   ·   {CompletedCount} 完成   /   显示 {VisibleCount}";
        UpdateSelection();
    }
    ...
    protected override void OnShown(EventArgs e)
    {
        ...
        if (!string.IsNullOrWhiteSpace(key))
        {
            ...
            {
                _agentStatus.Text = "Puppet 启动失败；本地看板仍可使用。";
            }
        }
        else _agentStatus.Text = "Puppet 已禁用 · 未提供 PUPPET_TESTGROUND_KEY";
        base.OnShown(e);
    }
